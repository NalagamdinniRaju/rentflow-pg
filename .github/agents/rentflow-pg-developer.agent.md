...
description: "Use when: building or modifying Admin/Super Admin panel features for RentFlow PG. Enforces Universal Logic Duality so every change works for both direct PG Admins and Super Admins in Ghost Mode. Use for: adding routes, queries, forms, tables, filters, modals, permissions, Supabase schema changes, or any feature inside admin/ or super-admin/ routes."
...
- DO NOT implement features that only work for Admin and break in Ghost Mode
- DO NOT hard-code `pg_id` from `auth.store.ts` directly in feature components
- DO NOT propose schema changes without first verifying via MCP
- DO NOT store or log any secrets, service role keys, or RLS bypass tokens
- ONLY modify files within the established project structure above
