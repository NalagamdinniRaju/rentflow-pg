...
export const registerKeys = {
  ...
  rooms: (floorId: string, roomTypeId?: string, sharingTypeId?: string) =>
    ["register", "rooms", floorId, roomTypeId, sharingTypeId] as const,
  seats: (roomId: string) => ["register", "seats", roomId] as const,
};

export const useRegistrationStates = createQuery<
...
export const useRegistrationCities = createQuery<
...
>({
  ...
  fetcher: async (variables) => {
    const res = await supabase
      ...
      .eq("state_id", variables.stateId)
      .order("name");
    return unwrapSupabaseResponse(res) as {
      id: string;
      name: string;
      state_id: string;
    }[];
...
export const useRegistrationBuildings = createQuery<
  {
    id: string;
    name: string;
...
export const useRegistrationRoomTypes = createQuery<
  { id: string; name: string }[]
>({
...
export const useRegistrationSharingTypes = createQuery<
...
export const useRegistrationFloors = createQuery<
  { id: string; floor_number: string }[],
  { buildingId: string }
>({
  ...
  fetcher: async (variables) => {
    ...
    const data = unwrapSupabaseResponse(res) as {
      id: string;
      floor_number: string;
    }[];
...
export const useRegistrationRooms = createQuery<
  {
    id: string;
    room_number: string;
...
>({
  ...
  fetcher: async (variables) => {
    let query = supabase
      .from("rooms")
      .select(
        "id, room_number, custom_monthly_rent, custom_daily_rent, custom_deposit_amount",
      )
...
export const useRegistrationSeats = createQuery<
...
export const useRegisterUser = createMutation<void, any>({
  mutationFn: async (variables) => {
    ...
    if (aadhar_file) {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("resident-documents").getPublicUrl(filePath);
      aadharUrl = publicUrl;
    }
